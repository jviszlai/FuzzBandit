...
havoc_stage:
  ...
  /* BANDITS: do one round of trying each of the havoc operations. */

  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {

    /* BANDITS: the different havoc stages. */
    ...
    switch (stage_cur) {
      ...
      case 13: {
        ...
        /* BANDITS: Do a simple input clone without a chance of blowing 
           up the input to HAVOC_BLK_XL size. */

        u32 clone_len = choose_block_len(temp_len);
        u32 clone_from = UR(temp_len - clone_len + 1);
        u32 clone_to = UR(temp_len);

        if (temp_len + clone_len >= max_file_len) {
          break;
        }

        u8* new_buf = ck_alloc_nozero(temp_len + clone_len);
        ...
        /* Free the old input and set the new input variables. */
